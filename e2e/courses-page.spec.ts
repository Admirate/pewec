...
test.describe("Courses Directory (/courses)", () => {
  test("shows links to long-term and short-term course pages", async ({ page }) => {
    await page.goto("/courses");
    await expect(page.locator('a[href="/courses/long-term"]')).toBeVisible();
    await expect(page.locator('a[href="/courses/short-term"]')).toBeVisible();
  });
});

test.describe("Long Term Courses (/courses/long-term)", () => {
  test("opens modal when clicking Enquire Now on a course card", async ({ page }) => {
    await page.goto("/courses/long-term");
...
test.describe("Short Term Courses (/courses/short-term)", () => {
  test("shows Enquire Now button on course cards", async ({ page }) => {
    await page.goto("/courses/short-term");
